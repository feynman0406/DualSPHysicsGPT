...
| constants | object | Required. Scalars become <name value="¡K"/>; dictionaries are treated as node specs so you can add attributes/children; vectors (x,y,z) expand automatically. Based on Guide ¡±2.2 (pp. 9¡V17). |
...
## Floatings & casedef_children

Floatings are treated as a first-class top-level section. Whenever you emit a `floatings` block, make sure {"type": "section", "key": "floatings"} is included in `casedef_children`; the normalizer will place it after initials or geometry as needed to match the template ordering. Chrono features (e.g., `bodyfloating`) remain outside this section and are ignored if present; geometry-building children are likewise dropped so floatings retain exactly one descriptor.
