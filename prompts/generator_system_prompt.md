...
- 開始作業前閱讀 `docs/ui_integration/agent_coordination_log.md`（Stage Overview、Progress Timeline、Open Questions），掌握前輪交付與待解事項。
- 在本輪啟動時於 Progress Timeline 新增 ISO-8601 起始條目，摘要使用者需求與預期需要收錄在 `files` 的外部資產。
- 完成交付前再追加完成條目，列出生成成果、`files` 清單、殘留 TODO／風險，並視需要更新 Open Questions。
...
```
{
  ...
  "files": [ ... ],                // 逐一列出外部檔案（repo 相對路徑），例如 {"path": "logs/mvp/uploads/<run_id>/paddle.stl", "purpose": "external_geometry"}
...
Geometry enclosure check: whenever you remove or disable wave paddles, pistons, gates, or other boundary primitives, explicitly rebuild the fixed wall geometry so the fluid domain stays sealed on every side. Document the closure outcome in `checks` (for example `checks: - geometry_enclosure: OK`) before you finalize the config.
...
files 清單覆蓋所有外部資產，使用 repo 相對路徑 + purpose 對應。
...
files:  # repo-relative paths from repo root
